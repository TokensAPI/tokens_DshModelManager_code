...
- **dsh gains `read_clipboard`, behind the harness's own approval gate.** The dsh plugin registers a second tool on macOS: any model — native DeepSeek included, no `(modlens vision)` variant needed, since the image never enters the message stream — can read the image on the system clipboard when the user says so. Every call answers `ask` on dsh's `tools/pre-execute` waterfall, so the composed approval service prompts the user before the clipboard is touched, and a session with no approval channel fails closed by dsh's own `serviceAsk` (verified live on a headless profile: the model reported the denied read and offered alternatives instead of fabricating image content). The tool speaks the two-phase protocol: a capture returns `{ snapshot, result }` under its own output schema (the result branch pinned to the vision schema by a lockstep test, routing meta preserved), and re-reads of the same image pass `snapshot.snapshotId`, which never re-reads the clipboard. Isolation holds: the tool, its schema, and the gate wiring live entirely inside the dsh plugin; `modlens clip` itself stays a harness-neutral CLI surface.

- **`modlens clip`: reading the clipboard directly, as a two-phase snapshot protocol.** "Read the screenshot I just took" previously had no honest answer: the clipboard is global mutable state, so a naive read can silently analyze whatever replaced the image between the user's copy and the model's command. `clip capture` freezes the clipboard once, in a single process that brackets the read with the macOS pasteboard change count (torn reads are discarded and retried, then fail as `CLIPBOARD_CAPTURE_RACE`), into an immutable snapshot: private directory, unguessable id, full SHA-256 identity, 30-minute TTL. Analysis runs through the normal provider chain and the evidence is stored beside the snapshot, so every later reference (`clip read <snapshotId>`) replays the confirmed image and never touches the clipboard again; a changed clipboard cannot drift a conversation that already confirmed what it saw, and a lost id fails safe into re-capture plus re-confirmation. Exactly one image is accepted per capture: raw PNG, TIFF (transcoded to PNG after the race window closes, both hashes reported separately), or one copied image file, magic-byte checked; multiple items, text, and oversized data each fail with their own identifier and without echoing clipboard contents. The skill gains the matching triggers ("read my clipboard", 剪贴板, 我刚截的图) and a confirmation step: the model repeats the captured summary back before relying on it, which is the only defense against the wrong-image case no tool can detect. macOS first; Windows and Linux report `CLIPBOARD_TOOL_MISSING` until their capture paths are verified on target machines. The v1 design died in review (an independent 12-finding rejection) and v2 is the rewrite that survived; `clip drop` cleans up early, the sweeper reaps expired snapshots lazily and grants in-flight captures a grace period instead of eating them.
