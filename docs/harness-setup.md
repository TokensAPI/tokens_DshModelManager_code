...
```sh
npx -y @deepseek-ai/dsh plugin --profile web add @tokens/dsh-model-manager@0.2.14
```
...
```sh
npx -y @deepseek-ai/dsh plugin --profile <name> add @tokens/dsh-model-manager@0.2.14
```

`npm view @tokens/dsh-model-manager version` prints the current one, and this page is
