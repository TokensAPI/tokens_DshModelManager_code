...
- `auto` (default `false`): auto mode. When `true`, a run first tries vision borrowed from other harness CLIs on this machine, in fast-first order: API credentials held by pi (borrowed into the inline path, guards intact), then a signed-in Codex CLI (`codex exec` with the image attached), then an OpenCode vision model (`opencode run`). Remote URLs never ride borrowed agents. An explicit `-p` still pins exactly one provider. Every borrowed answer adds a `meta.warnings` line naming whose quota it spent, and `modlens doctor`'s Auto section shows what is borrowable (probe results cache for 6 hours in `~/.modlens/auto-cache.json`; doctor always re-probes). Set with `modlens config set auto true`.
