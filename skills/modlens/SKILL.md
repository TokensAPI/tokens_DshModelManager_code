...
If your harness forbids running scripts, reason through the same order by hand and run the first line that works (the pinned version is 0.2.17):

1. A `modlens` on `PATH` whose major version is 0 and is at least 0.2.17: `modlens <args>`.
2. Otherwise, if `npx` exists: `npx --yes --package @tokens/dsh-model-manager@0.2.17 modlens <args>`.
3. Otherwise, if `bunx` exists: `bunx --bun @tokens/dsh-model-manager@0.2.17 <args>`.
