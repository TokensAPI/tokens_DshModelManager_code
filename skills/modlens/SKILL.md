...
If your harness forbids running scripts, reason through the same order by hand and run the first line that works (the pinned version is 3.18.6):

1. A `modlens` on `PATH` whose major version is 3 and is at least 3.18.6: `modlens <args>`.
2. Otherwise, if `npx` exists: `npx --yes --package @liustack/modlens@3.18.6 modlens <args>`.
3. Otherwise, if `bunx` exists: `bunx --bun @liustack/modlens@3.18.6 <args>`.
