...
- **Layered config**: CLI flags > `~/.modlens/config.json` (managed by `modlens config init/set/show`, 0600, masked rendering) > built-ins. Provider credentials come from the file alone since 3.17.0: `GEMINI_API_KEY`, `OPENAI_API_KEY`, `OPENAI_BASE_URL`, `ANTHROPIC_API_KEY` and `ANTHROPIC_BASE_URL` no longer configure anything, because an endpoint and a key are one credential and drawing halves from two places builds a pairing that exists in neither. modlens's own switches (`MODLENS_MODEL`, `MODLENS_HARNESS`) and the proxy conventions still read the environment.
