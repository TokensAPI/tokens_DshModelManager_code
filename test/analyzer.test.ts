...
describe('buildGeminiInvocation', () => {
  it('builds gemini -p invocation with absolute image path and model', () => {
    const invocation = buildGeminiInvocation({
      ...
      geminiBin: 'gemini',
      workspaceDir: '/Users/dev/projects/modlens',
    ...
    expect(invocation.cwd).toBe('/Users/dev/projects/modlens');
  });

  it('defaults cwd to image directory for workspace-safe file access', () => {
    const invocation = buildGeminiInvocation({
      imagePath: '/Users/dev/pictures/cases/case-01.png',
      geminiBin: 'gemini',
    });

    expect(invocation.cwd).toBe('/Users/dev/pictures/cases');
  });
});
